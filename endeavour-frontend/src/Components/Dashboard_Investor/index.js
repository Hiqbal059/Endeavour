...
import { GetIdeas, Signin, GetWorkshops, DeleteIdea, GetJoinedWorkShop, DeleteJoinedWorkShop, MarkFavorite, GetFavoriteideas, DeleteFavorite } from '../API/API';
...
const pages = ['Dashboard', 'Messages', 'Ideas', 'Favourite'];
const settings = [{
  name: "Edit Profile",
  link: "/edit-profile"
}, {
...
const Investor = () => {
  ...
  const handleDelete = (val) => {
    const response = DeleteFavorite({ id: val, username: localStorage.getItem("username") })
    response.then((data) => {
      toast.error("Idea removed from favorite")
      const all_ideas = GetFavoriteideas({ isAuthenticated: true, username: localStorage.getItem("username") })
      all_ideas
        .then((data) => {

          setIdeas(data.result)
          console.log("idea: ", data.result)
        })
        .catch((error) => {
          return error;
        });
  ...
  React.useEffect(() => {
    ...
    const all_ideas = GetFavoriteideas({ isAuthenticated: true, username: localStorage.getItem("username"), status: "active" })
    all_ideas
      .then((data) => {
        setIdeas(data.result)
  ...
  return (
    <>
          ...
          <Container fluid style={{ backgroundColor: ' #E5E5E5', padding: '40px 70px' }}>
            <Row>
              ...
              <Container style={{ backgroundColor: '#fff' }}>
                    <Row>
                        <Col md={12}>
                            <h6 style={{ padding: '10px 0px', fontSize: "20px", fontWeight: "bold", margin: "9px" }}> Favorite Ideas - ({ideas.length}) </h6>
                        </Col>
                    </Row>
                    <Row>
                        <Col md={12}>
                                    ...
                                    <Row className='m-auto' style={{ padding: '20px' }}>
                                        ...
                                        <Col md={3} className='m-auto'>
                                            ...
                                            >
                                                ...
                                                <button className='px-3 py-2 btn btn-danger d-block' style={{ textDecoration: 'none', color:"white", margin: "2px", width:"70%" }} onClick={() => handleDelete(idea.id)}>Remove</button>
                                            </Box>
                                        </Col>
                                    </Row>
                                </Container>
                                ...
                                <Container style={{ backgroundColor: '#fff' }}>
                                    <Row style={{ padding: '20px 10px' }}>
                                        <Col md={12}>
                                            <Row>
                                                <Col md={12}>
                                                    <h3 style={{ fontSize: '20px', fontWeight: "bold" }}> Workshops Joined({workshops.length}) </h3>
                                                </Col>
                                            </Row>
      ...
      {role !== "investor" && <div>You are not authenticated </div>}
