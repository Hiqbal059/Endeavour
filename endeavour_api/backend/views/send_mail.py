...
class SendMail(APIView):
    ...
    @classmethod
    def send_invite_to_participants(cls, data):
        """
        This function used to send email to agent
        """
        ...
        api_instance = sib_api_v3_sdk.TransactionalEmailsApi(sib_api_v3_sdk.ApiClient(cls.configuration))
        to = [{"email": data["email"], "name": data["user"].title()}]
        params = {"user":data["user"].title(),"link":data["link"], "title": data["title"].title(), "mentor": data["mentor"].title(), "templateId": 2}
        
        send_smtp_email = sib_api_v3_sdk.SendSmtpEmail(to=to, params=params, template_id=2)
